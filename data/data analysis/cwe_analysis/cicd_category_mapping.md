...
- Permission. Hijacked active sessions allow attackers to bypass authentication and gain unauthorized access to protected resources

## CWE-285:
- Not mapped.
- Improper Authorization
- The product does not perform or incorrectly performs an authorization check when an actor attempts to access a resource or perform an action.
- Permission. Deals directly with access control, user rights, and privilege validation

## CWE-407:
- Not mapped.
- Inefficient Algorithmic Complexity
- An algorithm in a product has an inefficient worst-case computational complexity that may be detrimental to system performance and can be triggered by an attacker, typically using crafted manipulations that ensure that the worst case is being reached.
- Coding Practices. When an application uses an algorithm that consumes excessive computational resources (like CPU or memory) when processing worst-case inputs, potentially leading to a Denial of Service (DoS)

## CWE-522:
- Not mapped.
- Insufficiently Protected Credentials
- The product transmits or stores authentication credentials, but it uses an insecure method that is susceptible to unauthorized interception and/or retrieval.
- Data protection. Fundamentally about securing sensitive data from unauthorized interception or retrieval.

## CWE-23:
- Not mapped.
- Relative Path Traversal
- The product uses external input to construct a pathname that should be within a restricted directory, but it does not properly neutralize sequences such as ".." that can resolve to a location that is outside of that directory.
- File Management. It involves insecure directory access, path resolution, and handling local files,

## CWE-288:
- Not mapped.
- Authentication Bypass Using an Alternate Path or Channel
- The product requires authentication, but the product has an alternate path or channel that does not require authentication.
- Permission. As it undermines authentication enforcement and access control mechanisms.

## CWE-367:
- Not mapped.
- Time-of-check Time-of-use (TOCTOU) Race Condition
- The product checks the state of a resource before using that resource, but the resource's state can change between the check and the use in a way that invalidates the results of the check.
- Coding Practices. This flaw stems from improper synchronization and flawed sequential logic in multi-threaded or concurrent environments.

## CWE-697:
- Not mapped.
- Incorrect Comparison
- The product compares two entities in a security-relevant context, but the comparison is incorrect.
- Coding Practices. This flaw is fundamentally a basic programming error rooted in flawed implementation logic.

## CWE-295: 
- Not mapped.
- Improper Certificate Validation
- The product does not validate, or incorrectly validates, a certificate.
- Data Protection. Allows attackers to perform Man-in-the-Middle (MitM) attacks to intercept, read, or tamper with sensitive data in transit, it fundamentally undermines secure communication.
